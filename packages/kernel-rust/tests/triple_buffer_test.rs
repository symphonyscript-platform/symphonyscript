...
fn writer_can_write_and_publish() {
    let sab = create_sab(4096);
    let (mut writer, _reader) = TripleBuffer::new(sab.clone(), 0, 4);

    let base = writer.current_start_index();
    sab[base].store(42, Ordering::Relaxed);
    sab[base + 1].store(99, Ordering::Relaxed);
    writer.publish();
}
...
fn reader_sees_published_data() {
    ...
    let base = writer.current_start_index();
    sab[base].store(100, Ordering::Relaxed);
    sab[base + 1].store(200, Ordering::Relaxed);
    sab[base + 2].store(300, Ordering::Relaxed);
    sab[base + 3].store(400, Ordering::Relaxed);
    ...
    assert_eq!(sab[rbase].load(Ordering::Relaxed), 100);
    assert_eq!(sab[rbase + 1].load(Ordering::Relaxed), 200);
    assert_eq!(sab[rbase + 2].load(Ordering::Relaxed), 300);
    assert_eq!(sab[rbase + 3].load(Ordering::Relaxed), 400);
}

#[test]
fn reader_swap_returns_false_when_no_new_data() {
    let sab = create_sab(4096);
    let (_writer, mut reader) = TripleBuffer::new(sab, 0, 4);
...
fn reader_swap_returns_true_when_new_data() {
    let sab = create_sab(4096);
    let (mut writer, mut reader) = TripleBuffer::new(sab, 0, 4);
...
fn reader_keeps_old_data_when_no_new_publish() {
    ...
    assert!(reader.swap());
    let rbase = reader.current_start_index();
    assert_eq!(sab[rbase].load(Ordering::Relaxed), 42);
    ...
    assert_eq!(sab[rbase2].load(Ordering::Relaxed), 42);
}
...
fn nonzero_start_index() {
    ...
    sab[base].store(55, Ordering::Relaxed);
    writer.publish();

    assert!(reader.swap());
    let rbase = reader.current_start_index();
    assert_eq!(sab[rbase].load(Ordering::Relaxed), 55);
}
...
fn buffer_size_of_one() {
    let sab = create_sab(4096);
    let (mut writer, mut reader) = TripleBuffer::new(sab.clone(), 0, 1);

    let base = writer.current_start_index();
    sab[base].store(777, Ordering::Relaxed);
    writer.publish();

    assert!(reader.swap());
    let rbase = reader.current_start_index();
    assert_eq!(sab[rbase].load(Ordering::Relaxed), 777);
}
...
fn writer_publishes_many_times_without_reader_consuming() {
    ...
    for i in 0..10 {
        let base = writer.current_start_index();
        sab[base].store(i * 100, Ordering::Relaxed);
        writer.publish();
    }
    ...
    assert!(reader.swap());
    let rbase = reader.current_start_index();
    assert_eq!(sab[rbase].load(Ordering::Relaxed), 900);
}
...
fn writer_reader_alternating_rapidly() {
    let sab = create_sab(4096);
    let (mut writer, mut reader) = TripleBuffer::new(sab.clone(), 0, 2);

    for i in 0..100 {
        let base = writer.current_start_index();
        sab[base].store(i, Ordering::Relaxed);
        sab[base + 1].store(i * 10, Ordering::Relaxed);
        writer.publish();

        assert!(reader.swap());
        let rbase = reader.current_start_index();
        assert_eq!(sab[rbase].load(Ordering::Relaxed), i);
        assert_eq!(sab[rbase + 1].load(Ordering::Relaxed), i * 10);
    }
}
...
fn writer_buffer_sync_after_publish() {
    ...
    let base1 = writer.current_start_index();
    sab[base1].store(111, Ordering::Relaxed);
    sab[base1 + 1].store(222, Ordering::Relaxed);
    sab[base1 + 2].store(333, Ordering::Relaxed);
    sab[base1 + 3].store(444, Ordering::Relaxed);
    ...
    assert_eq!(sab[base2].load(Ordering::Relaxed), 111);
    assert_eq!(sab[base2 + 1].load(Ordering::Relaxed), 222);
    assert_eq!(sab[base2 + 2].load(Ordering::Relaxed), 333);
    assert_eq!(sab[base2 + 3].load(Ordering::Relaxed), 444);
}
...
fn sync_correctness_across_multiple_publishes() {
    ...
    // Write field 0 = 10
    let base = writer.current_start_index();
    sab[base].store(10, Ordering::Relaxed);
    writer.publish();

    // After sync, writer's new buffer should have field 0 = 10
    let base = writer.current_start_index();
    assert_eq!(sab[base].load(Ordering::Relaxed), 10);

    // Now update field 1 = 20, keep field 0 as synced
    sab[base + 1].store(20, Ordering::Relaxed);
    writer.publish();

    // Both fields should persist in new writer
    let base = writer.current_start_index();
    assert_eq!(sab[base].load(Ordering::Relaxed), 10);
    assert_eq!(sab[base + 1].load(Ordering::Relaxed), 20);

    // Reader should see both fields
    reader.swap();
    let rbase = reader.current_start_index();
    assert_eq!(sab[rbase].load(Ordering::Relaxed), 10);
    assert_eq!(sab[rbase + 1].load(Ordering::Relaxed), 20);
}

#[test]
fn incremental_writes_dont_lose_prior_data() {
    let sab = create_sab(4096);
    let (mut writer, mut reader) = TripleBuffer::new(sab.clone(), 0, 8);

    // Write all 8 fields
    let base = writer.current_start_index();
    for i in 0..8 {
        sab[base + i].store((i + 1) as i32 * 100, Ordering::Relaxed);
    }
    writer.publish();
    reader.swap();
    ...
    let rbase = reader.current_start_index();
    for i in 0..8 {
        assert_eq!(sab[rbase + i].load(Ordering::Relaxed), (i + 1) as i32 * 100);
    }

    // Now only update field 3
    let base = writer.current_start_index();
    sab[base + 3].store(999, Ordering::Relaxed);
    writer.publish();
    reader.swap();
    ...
    assert_eq!(sab[rbase + 1].load(Ordering::Relaxed), 200);
    assert_eq!(sab[rbase + 2].load(Ordering::Relaxed), 300);
    assert_eq!(sab[rbase + 3].load(Ordering::Relaxed), 999); // updated
    assert_eq!(sab[rbase + 4].load(Ordering::Relaxed), 500);
    assert_eq!(sab[rbase + 5].load(Ordering::Relaxed), 600);
    assert_eq!(sab[rbase + 6].load(Ordering::Relaxed), 700);
    assert_eq!(sab[rbase + 7].load(Ordering::Relaxed), 800);
}

// ============ Bind (Reconnect to Existing SAB) ============
...
fn bind_reconnects_to_existing_state() {
    ...
    // Bind new reader to same SAB region
    let mut reader2 = TripleBuffer::bind_reader(sab.clone(), 0, 4);
...
fn bind_does_not_reinitialize_state() {
    ...
    let (mut writer, _reader) = TripleBuffer::new(sab.clone(), 0, 4);
    let base = writer.current_start_index();
    sab[base].store(99, Ordering::Relaxed);
    writer.publish();

    // Capture state before bind
    let state_before = sab[state_slot].load(Ordering::Relaxed);

    // Bind — should NOT overwrite state
    let _writer2 = TripleBuffer::bind_writer(sab.clone(), 0, 4);
    let _reader2 = TripleBuffer::bind_reader(sab.clone(), 0, 4);
    let state_after = sab[state_slot].load(Ordering::Relaxed);
...
fn dropped_frames_preserve_cumulative_state() {
    ...
    let base = writer.current_start_index();
    sab[base + 2].store(30, Ordering::Relaxed);
    writer.publish();
    ...
    assert_eq!(sab[rbase].load(Ordering::Relaxed), 10);
    assert_eq!(sab[rbase + 1].load(Ordering::Relaxed), 20);
    assert_eq!(sab[rbase + 2].load(Ordering::Relaxed), 30);
}
...
fn state_encoding_new_data_flag() {
    ...
    let state = sab[state_slot].load(Ordering::Relaxed);
...
fn large_buffer_data_integrity() {
    ...
    for i in 0..buffer_size {
        sab[base + i].store((i as i32) * 7 + 3, Ordering::Relaxed);
    }
    writer.publish();
    ...
    for i in 0..buffer_size {
        assert_eq!(
            sab[rbase + i].load(Ordering::Relaxed),
            (i as i32) * 7 + 3,
            "mismatch at index {i}"
        );
    }
...
fn sync_preserves_data_through_all_three_buffers() {
    ...
    for round in 0..6 {
        ...
        sab[base + field].store(expected[field], Ordering::Relaxed);
        ...
        for f in 0..4 {
            assert_eq!(
                sab[rbase + f].load(Ordering::Relaxed),
                expected[f],
...
fn concurrent_writer_reader_stress() {
    ...
    let sab_writer = sab.clone();
    let sab_reader = sab.clone();
    ...
    let writer_handle = thread::spawn(move || {
        for i in 0..iterations {
            ...
            for j in 0..buffer_size {
                sab_writer[base + j].store(i as i32, Ordering::Relaxed);
            }
            writer.publish();
        }
    });
        ...
        for _ in 0..iterations * 2 {
            if reader.swap() {
                ...
                for j in 1..buffer_size {
                    let val = sab_reader[rbase + j].load(Ordering::Relaxed);
                    assert_eq!(
                        val, first,
...
fn concurrent_high_frequency_publish() {
    ...
    let writer_handle = thread::spawn(move || {
        for i in 0..50_000i32 {
            let base = writer.current_start_index();
            sab_w[base].store(i, Ordering::Relaxed);
            writer.publish();
        }
    });
    ...
    let reader_handle = thread::spawn(move || {
        ...
        for _ in 0..5_000 {
            if reader.swap() {
                let rbase = reader.current_start_index();
                let _val = sab_r[rbase].load(Ordering::Relaxed);
                reads += 1;
            }
            thread::yield_now();
        }
...
fn writer_gets_back_coherent_buffer_after_reader_consumes() {
    ...
    let base = writer.current_start_index();
    for i in 0..4 {
        sab[base + i].store((i + 1) as i32, Ordering::Relaxed);
    }
    writer.publish();
    reader.swap();
    ...
    let base = writer.current_start_index();
    for i in 0..4 {
        sab[base + i].store((i + 5) as i32, Ordering::Relaxed);
    }
    writer.publish();
    reader.swap();
    ...
    let base = writer.current_start_index();
    assert_eq!(sab[base].load(Ordering::Relaxed), 5);
    assert_eq!(sab[base + 1].load(Ordering::Relaxed), 6);
    assert_eq!(sab[base + 2].load(Ordering::Relaxed), 7);
    assert_eq!(sab[base + 3].load(Ordering::Relaxed), 8);
}

#[test]
fn two_triple_buffers_on_same_sab() {
    ...
    sab[b1].store(1111, Ordering::Relaxed);
    w1.publish();

    let b2 = w2.current_start_index();
    sab[b2].store(2222, Ordering::Relaxed);
    w2.publish();
    ...
    assert_eq!(sab[rb1].load(Ordering::Relaxed), 1111);
    assert_eq!(sab[rb2].load(Ordering::Relaxed), 2222);
}
...
fn publish_swap_publish_swap_never_corrupts() {
    ...
    for round in 0..20 {
        ...
        for i in 0..4 {
            sab[base + i].store(accumulated[i], Ordering::Relaxed);
        }
...
#[test]
fn publish_does_not_corrupt_surrounding_sab_memory() {
    let padding = 100;
    let buffer_size = 64;
