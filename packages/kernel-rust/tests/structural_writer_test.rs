...
fn insert_lands_at_correct_mem_offset() {
    ...
    // The writer's current_start_index() gives us the actual MEM base.
...
fn copy_from_panics_if_source_larger() {
    ...
    let sw_large: StructuralWriter<16> = StructuralWriter::new(mem1, t_writer1, FL_START, 0, 16);

    let mem2 = create_mem(MEM_SIZE);
    let (t_writer2, _) = TripleBuffer::new(Arc::clone(&mem2), TB_START, TB_BUF_CAP);
    let sw_small: StructuralWriter<16> = StructuralWriter::new(mem2, t_writer2, FL_START, 0, 8);
