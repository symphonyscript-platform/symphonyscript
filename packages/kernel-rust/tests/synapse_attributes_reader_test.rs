...
fn synapse_attributes_reader_all_fields() {
    ...
    let writer =
        AttributePlaneWriter::<SYNAPSE_ATTRIBUTES_SLOT_SIZE>::new(sab.clone(), 0, CAPACITY);
    let reader = AttributePlaneReader::<SYNAPSE_ATTRIBUTES_SLOT_SIZE>::bind(sab, 0, CAPACITY);
