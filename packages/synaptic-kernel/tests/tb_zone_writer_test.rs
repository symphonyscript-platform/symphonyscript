...
fn construct_stride_1_at_offset_0() {
    ...
    let zone: TbZoneWriter<'_, 1> = TbZoneWriter::new(&writer, 0);
...
fn construct_stride_4_at_offset_0() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 4> = TbZoneWriter::new(&writer, 0);
    assert_eq!(zone.tb_start_offset(), 0);
...
fn construct_stride_64_at_offset_0() {
    ...
    let zone: TbZoneWriter<'_, 64> = TbZoneWriter::new(&writer, 0);
    assert_eq!(zone.tb_start_offset(), 0);
...
fn construct_at_small_nonzero_offset() {
    ...
    let writer = TripleBufferWriter::new(mem, 0, 512);
    let zone: TbZoneWriter<'_, 16> = TbZoneWriter::new(&writer, 1);
...
fn construct_at_large_nonzero_offset() {
    ...
    let zone: TbZoneWriter<'_, 64> = TbZoneWriter::new(&writer, 500);
...
fn construct_exact_fit_stride_1() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 1> = TbZoneWriter::new(&writer, 255);
...
fn construct_exact_fit_stride_16() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 16> = TbZoneWriter::new(&writer, 240);
...
fn panic_end_exceeds_capacity_by_one() {
    ...
    let _zone: TbZoneWriter<'_, 16> = TbZoneWriter::new(&writer, 17);
}
...
fn panic_start_equals_capacity_with_stride_1() {
    ...
    let _zone: TbZoneWriter<'_, 1> = TbZoneWriter::new(&writer, 16);
}
...
fn panic_far_out_of_bounds() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 16);
    let _zone: TbZoneWriter<'_, 16> = TbZoneWriter::new(&writer, 8);
}
...
fn panic_stride_exceeds_capacity() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 32);
    let _zone: TbZoneWriter<'_, 64> = TbZoneWriter::new(&writer, 0);
}
...
fn write_read_single_roundtrip_at_nonzero_offset() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 64);
...
fn zone_write_writes_at_start_offset_plus_index_in_underlying_tb() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 100);
...
fn disjoint_zones_at_nonzero_offsets_are_isolated() {
    ...
    let zone_a: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 32);
    let zone_b: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 40);
...
fn panic_on_read_offset_equal_to_stride() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 0);
    let _ = zone.read(8);
}
...
fn write_all_then_read_all_at_offset_1() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 1);

    let data: [i32; 8] = [11, 22, 33, 44, 55, 66, 77, 88];
    zone.write_all(data);
    assert_eq!(zone.read_all(), data);
}
...
fn write_all_then_read_all_at_offset_64() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 16> = TbZoneWriter::new(&writer, 64);
...
fn write_all_at_nonzero_offset_matches_individual_reads() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 32);
...
fn stride_1_write_all_read_all_at_nonzero_offset() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 256);
    let zone: TbZoneWriter<'_, 1> = TbZoneWriter::new(&writer, 199);

    zone.write_all([1234]);
    assert_eq!(zone.read_all(), [1234]);
...
fn stride_1_write_all_read_all_at_tail() {
    let mem = create_mem(1024);
    let writer = TripleBufferWriter::new(mem, 0, 64);
    let zone: TbZoneWriter<'_, 1> = TbZoneWriter::new(&writer, 63);

    zone.write_all([777]);
    assert_eq!(zone.read_all(), [777]);
}
...
fn full_capacity_zone_write_all_read_all() {
    ...
    let writer = TripleBufferWriter::new(mem, 0, 16);
    let zone: TbZoneWriter<'_, 16> = TbZoneWriter::new(&writer, 0);
...
fn write_all_in_one_zone_does_not_affect_other_zone_read_all() {
    ...
    let zone_a: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 32);
    let zone_b: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 64);
    ...
    assert_eq!(zone_a.read_all(), a_data);
    assert_eq!(zone_b.read_all(), b_data);
}
...
fn read_all_returns_only_own_region_on_multi_zone_layout() {
    ...
    let zone_a: TbZoneWriter<'_, 4> = TbZoneWriter::new(&writer, 0);
    let zone_b: TbZoneWriter<'_, 4> = TbZoneWriter::new(&writer, 4);
    let zone_c: TbZoneWriter<'_, 4> = TbZoneWriter::new(&writer, 8);

    zone_a.write_all([1, 1, 1, 1]);
    zone_b.write_all([2, 2, 2, 2]);
    zone_c.write_all([3, 3, 3, 3]);

    assert_eq!(zone_a.read_all(), [1, 1, 1, 1]);
    assert_eq!(zone_b.read_all(), [2, 2, 2, 2]);
    assert_eq!(zone_c.read_all(), [3, 3, 3, 3]);
}
...
fn adjacent_zones_do_not_bleed_at_boundary() {
    ...
    let left: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 16);
    let right: TbZoneWriter<'_, 8> = TbZoneWriter::new(&writer, 24);

    left.write_all([1, 2, 3, 4, 5, 6, 7, 8]);
    right.write_all([100, 200, 300, 400, 500, 600, 700, 800]);

    assert_eq!(left.read_all(), [1, 2, 3, 4, 5, 6, 7, 8]);
    assert_eq!(
        right.read_all(),
