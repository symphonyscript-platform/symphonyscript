...
fn setup_custom(
    ...
    synaptic_kernel::primitives::triple_buffer::TripleBufferReader,
    TopologyWriter<16>,
    TopologyReader<16>,
) {
    ...
    let sr: TopologyReader<16> = TopologyReader::new(reader.clone(), start_offset, CAPACITY);
...
fn reader_reads_from_correct_mem_offset() {
    ...
    // verify the reader's AtomicBuffer buffer actually contains the data
