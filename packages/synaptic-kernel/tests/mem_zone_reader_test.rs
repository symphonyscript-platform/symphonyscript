...
fn new_creates_reader_at_start_index() {
    ...
    let view: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 0);
...
fn reads_reflect_backing_buffer_state() {
    ...
    let view: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 10);
...
fn two_readers_share_same_backing_see_identical_values() {
    ...
    let view_a: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 10);
    let view_b: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 10);
...
fn independent_offsets_independent_data() {
    ...
    let view_a: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 0);
    let view_b: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 16);
...
fn works_with_different_stride_sizes() {
    ...
    let view_8: MemZoneReader<'_, 8> = MemZoneReader::new(&mem, 0);
    let view_16: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 64);
...
fn new_panics_if_out_of_bounds() {
    ...
    let _view: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 0);
}
...
fn new_panics_if_start_index_crosses_bounds() {
    ...
    let _view: MemZoneReader<'_, 16> = MemZoneReader::new(&mem, 20);
}
