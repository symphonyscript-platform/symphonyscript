...
describe('RollBuilder', () => {
  ...
  describe('basic emission', () => {
    ...
    it('should use bridge defaultDuration when duration not provided', () => {
      const bridge = createBridge({ defaultDuration: 480 })
      const result = roll(GM_DRUM.ACOUSTIC_SNARE).apply(bridge)
    ...
    it('should use bridge velocity for emitted notes', () => {
      ...
      const result = roll(GM_DRUM.COWBELL, 480, 120).apply(bridge)
  ...
  describe('duration and rate', () => {
    ...
    it('.rate() should set hit duration and thus hit count', () => {
      const bridge = createBridge({ defaultDuration: 480 })
      const result = roll(GM_DRUM.BASS_DRUM_1, 480, 240).apply(bridge)
    ...
    it('hitDuration defaults to defaultDuration/4 when rate not provided', () => {
      const bridge = createBridge({ defaultDuration: 480 })
      const result = roll(GM_DRUM.BASS_DRUM_1, 480).apply(bridge)
  ...
  describe('immutability', () => {
    it('builder methods should return new instances, not mutate', () => {
      const original = roll(GM_DRUM.BASS_DRUM_1, 480, 120)
  ...
  describe('chaining', () => {
    ...
    it('should chain with subsequent steps', () => {
      const bridge = createBridge({ defaultDuration: 480 })
      let b = roll(GM_DRUM.BASS_DRUM_1, 480, 120).apply(bridge)
      b = roll(GM_DRUM.ACOUSTIC_SNARE, 240, 120).apply(b)

      const { notes } = commitAndCapture(b)
      expect(notes).toHaveLength(6)
      expect(notes[4].pitch).toBe(GM_DRUM.ACOUSTIC_SNARE)
    })
  })
})
