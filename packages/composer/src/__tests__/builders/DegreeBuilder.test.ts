...
describe('DegreeBuilder', () => {
  ...
  describe('scale context', () => {
    it('degree(1) in G major should resolve to G4', () => {
      const bridge = createBridge({
        scaleRootCents: 6700, // G4 in test notation
        defaultDuration: 480,
      })
    ...
    it('degree(1) in A minor should resolve to A4', () => {
      const bridge = createBridge({
        scaleRootCents: 6900,
        scaleIntervals: [0, 200, 300, 500, 700, 800, 1000],
        defaultDuration: 480,
      })
      const { notes } = commitAndCapture(degree(1).apply(bridge))

      expect(notes[0].pitch).toBe(6900) // A4
    })
    ...
    it('degree(3) in A minor should resolve to C5 (minor third)', () => {
      ...
      expect(notes[0].pitch).toBe(7200) // C5
    })
  ...
  describe('inherited modifiers', () => {
    it('.velocity() should override bridge velocity', () => {
      const bridge = createBridge({ scaleRootCents: 6000, velocity: 600 })
  ...
  describe('chaining with note', () => {
    it('degree then note should both emit and advance tick', () => {
      ...
      expect(notes).toHaveLength(2)
      expect(notes[0].pitch).toBe(6000) // C4
      expect(notes[1].pitch).toBe(6700) // G4
      expect(notes[1].tick).toBe(480)
    })
  })
})
